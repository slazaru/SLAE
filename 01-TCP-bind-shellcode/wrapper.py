#!/usr/bin/python3

import argparse
import socket

parser = argparse.ArgumentParser()
parser.add_argument("port", help="Port number to bind on")
args = parser.parse_args()

shellcode = b""
shellcode += b"";

res = format(int(args.port), '04x')
res1 = res[0:2]
res2 = res[2:]
newbytes = bytearray.fromhex(res1 + res2)
#print(newbytes.hex())

#print(shellcode.hex())
#print(shellcode.replace(b"\x1b\x59", newbytes).hex())
newstr = ""
i = 0
for char in shellcode.replace(b"\x1b\x59", newbytes).hex():
    if (i % 2 == 0):
        newstr += "\\x"
        newstr += char
    else:
        newstr += char
    i += 1
print("shellcode for bind shell on {port}: ".format(port = args.port))
print(newstr)
